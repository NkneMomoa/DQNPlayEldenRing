...
class DQN():
    def __init__(self, observation_width, observation_height, action_space, model_file, loss_file):
        ...
        self.writer = tf.summary.FileWriter(self.loss_file, self.session.graph)
        # cd C:\Users\Momoa\PycharmProjects\pythonProject\Elden Ring
        # tensorboard --logdir=loss_gpu --host=127.0.0.1
    ...
    def create_Q_network(self):
        ...
        with tf.variable_scope('current_net'):
            ...
            # dropout
            h_layer_one = tf.nn.dropout(h_layer_one, rate=0.5)
            # hidden layer two
            h_layer_two = tf.nn.relu(tf.matmul(h_layer_one, W2) + b2)
            # dropout
            h_layer_two = tf.nn.dropout(h_layer_two, rate=0.25)
            # the output of current_net
            Q_value = tf.matmul(h_layer_two, W3) + b3
            # dropout
            self.Q_value = tf.nn.dropout(Q_value, rate=0.25)
        ...
        with tf.variable_scope('target_net'):
            ...
            # dropout
            t_h_layer_one = tf.nn.dropout(t_h_layer_one, rate=0.5)
            # 防止过拟合
            # hidden layer two
            t_h_layer_two = tf.nn.relu(tf.matmul(t_h_layer_one, t_W2) + t_b2)
            # dropout
            t_h_layer_two = tf.nn.dropout(t_h_layer_two, rate=0.25)
            # the output of current_net
            target_Q_value = tf.matmul(t_h_layer_two, t_W3) + t_b3
            # dropout
            self.target_Q_value = tf.nn.dropout(target_Q_value, rate=0.25)
